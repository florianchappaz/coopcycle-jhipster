...
import com.coopcycle.service.CityService;
...
/**
 * REST controller for managing {@link com.coopcycle.domain.City}.
 */
@RestController
@RequestMapping("/api")
public class CityResource {
    ...
    public List<CityDTO> getAllCities(@RequestParam(required = false) String filter) {
        if ("customer-is-null".equals(filter)) {
            log.debug("REST request to get all Citys where customer is null");
            return cityService.findAllWhereCustomerIsNull();
        }

        if ("restaurant-is-null".equals(filter)) {
            log.debug("REST request to get all Citys where restaurant is null");
            return cityService.findAllWhereRestaurantIsNull();
        }
