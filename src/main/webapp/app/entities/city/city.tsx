  ...
  return (
    <div>
      <h2 id="city-heading" data-cy="CityHeading">
        <Translate contentKey="coopcycleApp.city.home.title">Cities</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="coopcycleApp.city.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="coopcycleApp.city.home.createLabel">Create new City</Translate>
          </Link>
        </div>
      </h2>
        ...
        {cityList && cityList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="coopcycleApp.city.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {cityList.map((city, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{city.name}</td>
                  <td>{city.zipCode}</td>
                  <td>{city.zone ? <Link to={`zone/${city.zone.id}`}>{city.zone.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="coopcycleApp.city.home.notFound">No Cities found</Translate>
            </div>
          )
        )}
      </div>
